...
<template>
  <div class="navbar flex_r_c">
    <div @click="emitSiderbarStatus" class="navbar-mune flex_c_c">
      <div class="sidebar-toggler flex_c_c" :class="[
          {
    ...
    <div @mouseenter="addHoverClass" @mouseleave="removeHoverClass" class="navbar-menu"
      v-if="false">
      <span class="cur_P"
        style="display: inline-block; height: 100%; padding: 0 15px">{{ $i18n('layouts.components.navbar.云产品') }}</span>
      <div class="li-inner navbar-menuinner theme_container">
        <span @click="linkTo('/scf/list')"
          class="cur_P">{{ $i18n('layouts.components.navbar.云函数') }}</span>
      </div>
    </div>
    <!--navbar menu -->
    ...
    <ul class="navbar-account flex_r_c">
      <li class="navbar-account-li">
        <ui-icon name="set" color="#fff" width="22px"></ui-icon>
      ...
      <li class="navbar-account-li">
        ...
        <ui-popover trigger="hover">
          ...
          <ui-popover-cell @click.native="postLogout">
            ...
            <span style="padding-left: 10px">{{ $i18n('layouts.components.navbar.退出登录') }}</span>
          </ui-popover-cell>
        </ui-popover>
      </li>
    </ul>
...
<style lang="scss">
  ...
  .navbar {
    ...
    .sidebar-toggler {
      ...
      span:nth-child(1) {
        animation: top8 0.6s ease forwards;
      }
      ...
      span:nth-child(3) {
        animation: bottom8 0.6s ease forwards;
      }
    }

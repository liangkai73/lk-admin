  ...
  >
    <div class="store-detail-base mt20">
      <div class="detail-base-info">
        <p style="font-size: 18px; font-weight: 600">{{ $i18n('productCenter.cpu.detail.基本信息') }}</p>
        <ui-row class="mt20">
          <ui-col :span="8">
            <p class="info-title">{{ $i18n('productCenter.cpu.detail.名称') }}</p>
            <p>{{ app_info.name }}</p>
          </ui-col>
          <ui-col :span="8">
            <p class="info-title">{{ $i18n('productCenter.cpu.detail.所属分组') }}</p>
      ...
      <div v-if="is_empty" class="flex_c_c" style="height: 360px">
        <bp-icon name="Failed" style="color: rgba(0, 0, 0, 0.38);font-size: 30px"></bp-icon>
...
export default class extends Vue {
  ...
  baseInfoTabsArr = [
    {
      name: $i18n('productCenter.cpu.detail.CPU使用率'),
      value: 1,
    },
  ];
  ...
  time_group = [
    { name: $i18n('productCenter.cpu.detail.1小时'), value: 60 * 60 * 1000, choose: false },
    { name: $i18n('productCenter.cpu.detail.3小时'), value: 3 * 60 * 60 * 1000, choose: false },
    { name: $i18n('productCenter.cpu.detail.6小时'), value: 6 * 60 * 60 * 1000, choose: false },
    { name: $i18n('productCenter.cpu.detail.12小时'), value: 12 * 60 * 60 * 1000, choose: false },
    { name: $i18n('productCenter.cpu.detail.1天'), value: 24 * 60 * 60 * 1000, choose: true },
    { name: $i18n('productCenter.cpu.detail.3天'), value: 3 * 24 * 60 * 60 * 1000, choose: false },
    { name: $i18n('productCenter.cpu.detail.7天'), value: 7 * 24 * 60 * 60 * 1000, choose: false },
    { name: $i18n('productCenter.cpu.detail.14天'), value: 14 * 24 * 60 * 60 * 1000, choose: false },
    { name: $i18n('productCenter.cpu.detail.30天'), value: 30 * 24 * 60 * 60 * 1000, choose: false },
  ];
  //
  // computed.

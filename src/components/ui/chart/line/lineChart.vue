...
export default {
  ...
  methods: {
    ...
    // 获取配置数据 x 轴数据 Y轴数据
    getOptions({ xAxisData, seriesData, xAxis, yAxis } = {}) {
      // 数据格式化
      let toolbox = {
        show: this.brush,
      };
      let seriesDatas = seriesData.map((item) => {
        return {
          name: item.name,
          smooth: this.smooth,
          type: "line",
      ...
      let options = febs.utils.mergeMap(default_option, {
        toolbox: toolbox,
        legend: {
          data: legends,
        },
      ...
      options.yAxis = { type: "value", show: true };
      if (yAxis) {
        if (yAxis.show) options.yAxis.show = yAxis.show;

        if (yAxis.min) options.yAxis.min = yAxis.min;

        if (yAxis.max) options.yAxis.min = yAxis.max;
      }
      options.series = seriesDatas;
      seriesDatas.forEach((element) => {
        element.areaStyle = {};
        element.smooth = true;
      });

      options = febs.utils.mergeMap(options, this.extraOption);
      return options;
    },
  },
};

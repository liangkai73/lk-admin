<template>
  <div>
    <ui-select :disabled="disabled" :empty-text='noDataText' :placeholder="placeholderTxt"
      @blur="judgeMust" :clearable="clearable" @change="noticChange" style="width:100%;margin:0"
      v-if="showIcon" v-model="insideValue">
      <ui-select-option :key="i" :value="item[paintVal]" v-for="(item,i) in propList">
        <span>
    ...
    <ui-select v-else class="baseSelect" :class="{hasVal:insideValue?insideValue.toString():false}"
      popper-class="downFrame" :placeholder="placeholderTxt" @change="noticChange" @blur="judgeMust"
      :filterable="filterable" :collapse-tags="collapseTags" :clearable="clearable"
      :disabled="disabled" :no-data-text='noDataText' :multiple-limit="multipleLimit"
      :multiple="multiple" v-model="insideValue">
      <template v-if="paintLabel">
        <ui-select-option :key="i" :value="item[paintVal]" :label="item[paintLabel]"
          v-for="(item,i) in propList">
          {{$i18n(item[paintLabel])}}
        </ui-select-option>
      </template>
      <template v-else>
        <ui-select-option :key="item" :label="item" :value="item" v-for="item in propList"></ui-select-option>
      </template>
    </ui-select>
    <div class="requiredTip" v-show="requiredTipShow && judgeEmpty">
      {{placeholderTxt}}{{ '必填' }}</div>
  </div>
</template>
<script>

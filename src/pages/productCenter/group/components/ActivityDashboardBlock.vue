  ...
  >
    <p class="flex_r_c dashboard-block__title">
      ...
      <span class="dashboard-block__title__control" @click="onClickBlock"
        >{{ $i18n('productCenter.group.components.ActivityDashboardBlock.查看') }}</span
      >
    </p>
    <p class="flex_r_c dashboard-block__title_subhead">
      <span>{{ $i18n('productCenter.group.components.ActivityDashboardBlock.总计') }}</span>
      <span>{{ total }}{{ $i18n('productCenter.group.components.ActivityDashboardBlock.个') }}</span>
    </p>
    ...
    <div class="dashboard-block__content_list">
      <p class="bp-ellipsis" v-for="(item, index) in list_events" :key="index">
        {{ item.eventMsg }}
      </p>
    </div>
  </ui-card>
...
export default class extends DashboardBlockMixins {
  ...
  mounted() {
    ...
    this.title = $i18n('productCenter.group.components.ActivityDashboardBlock.事件');

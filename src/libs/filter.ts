...
function simplify(val) {
  let amount: any = $Febs.string.replace(val, ",", "");
  -0;
  if ($UILibs.lang === "en") {
    if (amount > 999_999) {
      amount = (amount / 1_000_000).toFixed(2) + "M";
    } else if (amount > 999) {
...
function filterMoney(
  ...
  if (typeof money == "string" || typeof money == "object") {
    money = parseFloat(money as any);
  }
  isNaN(money) && (money = 0);

  return type + money.toFixed(2);
}
...
function cloudRelName(str: string) {
  const alName = {
    ALIYUN: "阿里云",
    HUAWEI: "华为云",
    TENCENT: "腾讯云",

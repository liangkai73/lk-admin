...
<template>
  <content-view
    ...
    :title="$i18n('notification.system.index.系统事件')"
  >
    <div slot="titleRight">
      <div class="flex_r_s">
          <div style="width: 100px" class="flex_c_c">{{ $i18n('notification.system.index.事件类型') }}</div>

        <div style="width: 200px">
          <form-select
            v-model="type_choose"
            :placeholder-txt="$i18n('notification.system.index.事件类型')"
            paint-label="title"
            paint-val="key"
            :clearable="true"
         ...
         <div style="width: 100px" class="flex_c_c">{{ $i18n('notification.system.index.事件来源') }}</div>

        <div style="width: 200px">
          <form-select
            v-model="service_choose"
            :placeholder-txt="$i18n('notification.system.index.事件来源服务')"
            paint-label="title"
            paint-val="key"
            :clearable="true"
      ...
      <ui-table>
        <ui-thead>
          <ui-tr>
            ...
            <ui-th width="20%">{{ $i18n('notification.system.index.事件源') }}</ui-th>
            <ui-th width="20%">{{ $i18n('notification.system.index.用户') }}</ui-th>
            <ui-th width="20%">{{ $i18n('notification.system.index.内容') }}</ui-th>
            <ui-th width="20%">{{ $i18n('notification.system.index.时间') }}</ui-th>
          </ui-tr>
        </ui-thead>
        <ui-tbody>
          <ui-tr :key="index" v-for="(item, index) in list_events" @click="linkTo(item)">
            <ui-td>
              <div>
                <!-- <bp-icon
...
@Component({
  components: { contentView, uiPagination, bpInput: bpui.bpInput, sideViewDetail, formSelect },
})
export default class extends Vue {
  ...
  mounted() {
    ...
    api.event.event
      .getEventsServices({ classify: "SYS" })
      .then((data: any) => {
